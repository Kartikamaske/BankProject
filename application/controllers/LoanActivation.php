...
$output.='<td ></td>';
	...
	public function Report()
	{
		...
		$data['data']=$this->Model_LoanActivation->loanactivationdata();
        
		// echo "<pre>";
		// print_r($data);
		$this->load->view('common/header_view');
		$this->load->view("LoanActivation/LoanActivation_Reportview",$data);
		$this->load->view('common/footer_view');

	}


	public function loactAllData()
    {
        $fkmemberId = $this->input->post('fkmemberId'); 
    
        $data = $this->Model_LoanActivation->loactAllData($fkmemberId);
        echo json_encode($data);
    }
	
	public function getAllRefrences()
    {
        $fkmemberId = $this->input->post('fkmemberId'); 
    
        $data = $this->Model_LoanActivation->getAllRefrences($fkmemberId);
        echo json_encode($data);
    }

	public function getAllInstallmentdate()
    {
        $fkmemberId = $this->input->post('fkmemberId');
    
        $data = $this->Model_LoanActivation->getAllInstallmentdate($fkmemberId);
        echo json_encode($data);
    }

	
	public function getinstallmentAmt()
    {
        $fkloanActivationId = $this->input->post('fkloanActivationId');
    
        $data = $this->Model_LoanActivation->getinstallmentAmt($fkloanActivationId);
        echo json_encode($data);
    }
	...
	function updatePayNow(){
		...
		$mInstallmentDatesId= $this->input->post('mInstallmentDatesId'); 
		$paidinstallment= $this->input->post('paidinstallment'); 

	     $paidpenalty= $this->input->post('paidpenalty'); 
		...
		$fields=array(
			...
			'paidpenalty'=>$paidpenalty,
	          ...
	          'modified_date'=>date('Y-m-d H:i:s'),
		      'modified_by'=>1);
			  $whereArray=array('mInstallmentDatesId'=>$mInstallmentDatesId);

			  $this->Commonmodel->updateRecord("loan_installmentdates",$fields,$whereArray);
