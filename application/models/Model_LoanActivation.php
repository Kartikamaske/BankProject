...
if( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	{					
		// $sql=$this->db->get_where('memberregistration_master',['is_active'=>1]);
		// 	return $sql->result();
...
// ******************  loan activation dropdown data fetch***********

// public function loanactivationdata($fullName,$mobileNo)
// {	


// 	$this->db->select('loan_activation.loanActivationId,loan_activation.fkmemberId,memberregistration_master.mobilenumber,memberregistration_master.fullname');

// 	$this->db->join('memberregistration_master','memberregistration_master.memberid = loan_activation.fkmemberId','left outer');

// 	$this->db->like('memberregistration_master.fullname',$fullName);
// 	$this->db->or_like('memberregistration_master.mobilenumber',$mobileNo);
// 	$this->db->from('loan_activation');
// 	$this->db->where('loan_activation.is_active',1);
// 	$this->db->ORDER_BY('loan_activation.loanActivationId','desc'); 
// 	$this->db->limit(10); 
...
public function loanactivationdata(){
...
public function loactAllData($fkmemberId)
		{
			$this->db->select('loan_activation.loanActivationId,loan_activation.fkloantypeId,loantype_master.loantype_name,DATE_FORMAT(loan_activation.startDate,"%d/%m/%Y") as startDate,DATE_FORMAT(loan_activation.endDate,"%d/%m/%Y") as endDate,installmenttype_master.installmentTypeName,loan_activation.installmentno,loan_activation.excludeorinclude,loan_activation.installmentAmt,loan_activation.interestper,loan_activation.haptars,loan_activation.penaltyAmt,loan_activation.interestamt,loan_activation.interetotalstamt,loan_activation.interestamttotal,loan_activation.totalInstallmentAmt');
			...
			$this->db->from('loan_activation');
			$this->db->where('loan_activation.loanActivationId', $fkmemberId);
			$query = $this->db->get();
			return $query->result();
		}
		...
		public function getAllRefrences($fkmemberId){

			$this->db->select("loan_referancedata.referanceId,loan_referancedata.fkmemberId,memberregistration_master.fullname,loan_referancedata.refmobile,loan_referancedata.fkrelationId,loan_referancedata.fklicencetypeId,relation_master.relationName,loan_referancedata.attachment,loan_referancedata.loanInstaDateId,loan_referancedata.fklicencetypeId");
			...
			$this->db->from('loan_referancedata');
			$this->db->where('fkLoanActivationId',$fkmemberId);
			$this->db->where('loan_referancedata.is_active',1);
			$query=$this->db->get();
			return $query->result();
		}
		...
		public function getAllInstallmentdate($fkmemberId){

			$this->db->select("loan_installmentdates.mInstallmentDatesId,loan_installmentdates.fkmemberId,DATE_FORMAT(loan_installmentdates.startDate,'%d/%m/%Y') as startDate,DATE_FORMAT(loan_installmentdates.endDate,'%d/%m/%Y') as endDate  ,fkloanActivationId,loan_installmentdates.is_paid,loan_installmentdates.paidpenalty,loan_activation.haptars,loan_installmentdates.endDate as endDate1");

		$this->db->join('loan_activation','loan_activation.loanActivationId = loan_installmentdates.fkloanActivationId','left outer');

			$this->db->from('loan_installmentdates');
			$this->db->where('loan_installmentdates.fkloanActivationId',$fkmemberId);
			$this->db->where('loan_installmentdates.is_active',1);
			$query=$this->db->get();
			return $query->result();
		}

		public function getinstallmentAmt($fkloanActivationId){

			$this->db->select("loan_activation.loanActivationId,loan_activation.penaltyAmt,loan_activation.haptars");

		

			$this->db->from('loan_activation');
			$this->db->where('loan_activation.loanActivationId',$fkloanActivationId);
			$this->db->where('loan_activation.is_active',1);
			$query=$this->db->get();
			return $query->result();
		}
